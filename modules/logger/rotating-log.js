var RotatingLog = function(){
	...
	*/
	...
	var saveRecords = function(){
		...
		var newTab = window.open(encodedUri, '_blank');
		messages = [];
	}
	...
	this.setup = function(options){
		...
		*/
		if (typeOfMaxSize !== 'undefined'){
			if (typeOfMaxSize !== 'number'){
				throw 'RotatingLog :: illegal type "' + typeOfMaxSize + '" for maxSize, "number" expected';
			} else {
		...
		*/
		if (typeOfSliding !== 'undefined'){
			if (typeOfSliding !== 'boolean'){
				throw 'RotatingLog :: illegal type "' + typeOfSliding + '" for sliding, "boolean" expected';
			} else {
		...
		*/
		if (typeOfRecEnabled !== 'undefined'){
			if (typeOfRecEnabled !== 'boolean'){
				throw 'RotatingLog :: illegal type "' + typeOfRecEnabled + '" for recordingEnabled, "boolean" expected';
			} else {
        ...
        */
        ...
        if (valueType === 'function'){
            ...
            throw 'RotatingLog :: illegal type for saveRecords - expected function, got ' + valueType;
        }
